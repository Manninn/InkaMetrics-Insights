...
public class CanalController {
    ...
    public ResponseEntity<List<CanalDTO>> ranking(@PathVariable int id) {

        ModelMapper m = new ModelMapper();

        List<CanalDTO> lista = cS.rankingPorPlataforma(id).stream().map(c -> {
            CanalDTO dto = m.map(c, CanalDTO.class);
            dto.setPlataforma(c.getPlataforma());
            return dto;
        }).toList();
    ...
    public ResponseEntity<List<CanalDTO>> buscarPorSeguidores(@PathVariable int min) {

        ModelMapper m = new ModelMapper();

        List<CanalDTO> lista = cS.buscarPorSeguidores(min).stream().map(c -> {
            CanalDTO dto = m.map(c, CanalDTO.class);
            dto.setPlataforma(c.getPlataforma());
            return dto;
        }).toList();
