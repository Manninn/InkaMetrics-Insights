...
public class PlataformaController {
    ...
    public ResponseEntity<List<PlataformaDTO>> buscarPorNombre(@PathVariable String nombre) {

        ModelMapper m = new ModelMapper();

        List<PlataformaDTO> lista = plS.buscarPorNombre(nombre).stream()
                .map(p -> m.map(p, PlataformaDTO.class))
                .toList();
