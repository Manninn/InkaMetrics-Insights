...
public interface IMetricaSnapshotRepository extends JpaRepository<MetricaSnapshot,Integer> {
    ...
    @Query("SELECT new pe.edu.upc.tpbackinkametrics.dtos.MetricaPorRegionDTO(r.Nombre, m.Nombre, AVG(m.Cantidad)) " +
            "FROM MetricaSnapshot m JOIN m.Transmision t JOIN t.canal c JOIN c.Streamer s JOIN s.region r " +
